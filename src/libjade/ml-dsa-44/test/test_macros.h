...
#define check_pk_rhot1(FUNCTION_C, FUNCTION_JASMIN, function_name) \
  ...
  for(int t=0; t<TESTS; t++) {	 \
    ...
    eqarr(JASMIN_MLDSA44_CRYPTO_PUBLICKEYBYTES, PRId8, pk, pk_jazz, function_name) \
  } \
  printf("PASS: %s\n", function_name); \
  } 
...
#define check_rhot1_pk(FUNCTION_C, FUNCTION_JASMIN, function_name) \
  ...
  uint8_t pk_jazz[JASMIN_MLDSA44_CRYPTO_PUBLICKEYBYTES]; \
  for(int t=0; t<TESTS; t++) {	 \
    fillarrnu8(pk, pk_jazz, JASMIN_MLDSA44_CRYPTO_PUBLICKEYBYTES); \
  ...
  { \
    ...
    for(int t=0; t<TESTS; t++) {	 \
        ...
        eqarr(JASMIN_MLDSA44_CRYPTO_SECRETKEYBYTES, PRId8, sk, sk_jazz, function_name) \
    } \
    printf("PASS: %s\n", function_name); \
  }
  ...
  { \
    ...
    uint8_t sk_jazz[JASMIN_MLDSA44_CRYPTO_SECRETKEYBYTES]; \
    for(int t=0; t<TESTS; t++) {	 \
        fillarrnu8(sk, sk_jazz, JASMIN_MLDSA44_CRYPTO_SECRETKEYBYTES); \
