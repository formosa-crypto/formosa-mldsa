...
void check_crypto_sign_signature_ctx() {
  ...
  for(int t=0; t<TESTS; t++) {
    ...
    int r = PQCLEAN_MLDSA44_CLEAN_crypto_sign_signature_ctx(sig, &siglen, m, mlen, ctx, ctxlen, sk);
    if (r != 0) {
      printf("FAIL: crypto_sign_signature. Could not generate the signature with C code.");
      exit(1);
    }
    int r_jazz = JASMIN_MLDSA44_crypto_sign_signature_ctx(sig_jazz, &siglen_jazz, m_jazz, mlen, ctx_jazz, ctxlen, sk);
    if (r_jazz != 0) {
      printf("FAIL: crypto_sign_signature. Could not generate the signature with jasmin code.");
      exit(1);
    }
    eqarr(PQCLEAN_MLDSA44_CLEAN_CRYPTO_BYTES, PRId8, sig, sig_jazz, "crypto_sign_signature_ctx");
  }
  printf("PASS: crypto_sign_signature\n");
}

void check_crypto_sign_verify_ctx() {
  size_t siglen = PQCLEAN_MLDSA44_CLEAN_CRYPTO_BYTES;
  size_t mlen = 453;
  size_t ctxlen = 239;
  uint8_t m[mlen];
  uint8_t ctx[ctxlen];
  ...
  for(int t=0; t<TESTS; t++) {
    ...
    // Sign the random message
    status = PQCLEAN_MLDSA44_CLEAN_crypto_sign_signature_ctx(sig, &siglen, m, mlen, ctx, ctxlen, sk);
    if (status != 0) {
      printf("FAIL: crypto_sign_verify. Could not generate the signature.\n");
      exit(1);
    }
    // Verify the maching between the signature and the message
    int r = PQCLEAN_MLDSA44_CLEAN_crypto_sign_verify_ctx(sig, PQCLEAN_MLDSA44_CLEAN_CRYPTO_BYTES, m, mlen, ctx, ctxlen, pk);
    if (r != 0) {
      printf("Could not verify signature with C code.");
      exit(1);
    }
    int r_jazz = JASMIN_MLDSA44_crypto_sign_verify_ctx(sig, PQCLEAN_MLDSA44_CLEAN_CRYPTO_BYTES, m, mlen, ctx, ctxlen, pk);
    if (r != r_jazz) {
