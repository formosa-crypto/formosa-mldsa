...
void check_crypto_sign_keypair() {
  ...
  for (int t=0; t<TESTS; t++) {
    ...
    JASMIN_MLDSA65_crypto_sign_keypair(pk_jazz, sk_jazz);
    ...
    int r_sign = PQCLEAN_MLDSA65_CLEAN_crypto_sign_signature_ctx(sig, &siglen, m, mlen, ctx, ctxlen, sk_jazz);
    if (r_sign != 0) {
    ...
    int r_verify = PQCLEAN_MLDSA65_CLEAN_crypto_sign_verify_ctx(sig, PQCLEAN_MLDSA65_CLEAN_CRYPTO_BYTES, m, mlen, ctx, ctxlen, pk_jazz);
    if (r_verify != 0) {
