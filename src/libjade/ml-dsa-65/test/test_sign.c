...
void JASMIN_MLDSA65_crypto_sign_keypair(uint8_t *pk, uint8_t *sk);
...
void check_crypto_sign_signature_ctx_seed() {
  ...
  for(int t=0; t<TESTS; t++) {
    ...
    int r = PQCLEAN_MLDSA65_CLEAN_crypto_sign_signature_ctx_seed(sig, &siglen, m, mlen, ctx, ctxlen, sk, rand);
    if (r != 0) {
      printf("FAIL: crypto_sign_signature_ctx_seed. Could not generate the signature with C code.");
      exit(1);
    }
    int r_jazz = JASMIN_MLDSA65_crypto_sign_signature_ctx_seed(sig_jazz, &siglen_jazz, m_jazz, mlen, ctx_jazz, ctxlen, sk, rand_jazz);
    if (r_jazz != 0) {
      printf("FAIL: crypto_sign_signature_ctx_seed. Could not generate the signature with jasmin code.");
      exit(1);
    }
    eqarr(PQCLEAN_MLDSA65_CLEAN_CRYPTO_BYTES, PRId8, sig, sig_jazz, "crypto_sign_signature_ctx_seed");
  }
...
void check_crypto_sign_keypair() {
  ...
  for (int t=0; t<TESTS; t++) {
    ...
    JASMIN_MLDSA65_crypto_sign_keypair(pk, sk);
    ...
    // Sign the random message
    int r_sign = PQCLEAN_MLDSA65_CLEAN_crypto_sign_signature_ctx(sig, &siglen, m, mlen, ctx, ctxlen, sk);
    if (r_sign != 0) {
    ...
    int r_verify = PQCLEAN_MLDSA65_CLEAN_crypto_sign_verify_ctx(sig, PQCLEAN_MLDSA65_CLEAN_CRYPTO_BYTES, m, mlen, ctx, ctxlen, pk);
    if (r_verify != 0) {
...
void check_crypto_sign_signature() {
  ...
  for (int t=0; t<TESTS; t++) {
    ...
    // Sign the random message
    int r_signature = JASMIN_MLDSA65_crypto_sign_signature_ctx(sig, &siglen, m, mlen, ctx, ctxlen, sk);
    if (r_signature != 0) {
...
void check_crypto_sign_verify() {
  ...
  for (int t=0; t<TESTS; t++) {
    ...
    // Sign the random message
    int r_signature = PQCLEAN_MLDSA65_CLEAN_crypto_sign_signature_ctx(sig, &siglen, m, mlen, ctx, ctxlen, sk);
    if (r_signature != 0) {
    ...
    // Verify the maching between the signature and the message
    int r_verify = JASMIN_MLDSA65_crypto_sign_verify_ctx(sig, PQCLEAN_MLDSA65_CLEAN_CRYPTO_BYTES, m, mlen, ctx, ctxlen, pk);
    if (r_verify != 0) {
