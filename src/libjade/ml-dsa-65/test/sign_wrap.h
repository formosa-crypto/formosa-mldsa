...
extern int JASMIN_MLDSA65_crypto_sign_verify_ctx_internal(const uint8_t *sig, const uint8_t* ctx_m[2], size_t ctxlen_mlen[2], const uint8_t *pk);
						     ...
						     const uint8_t *pk)
{
  ...
  ctxlen_mlen[0] = ctxlen;
  ctxlen_mlen[1] = mlen;

  return JASMIN_MLDSA65_crypto_sign_verify_ctx_internal(sig, ctx_m, ctxlen_mlen, pk);
}
