...
void check_crypto_sign_keypair() {
  ...
  for (int t=0; t<TESTS; t++) {
    ...
    JASMIN_MLDSA87_crypto_sign_keypair(pk, sk);
    ...
    // Sign the random message
    int r_sign = PQCLEAN_MLDSA87_CLEAN_crypto_sign_signature_ctx(sig, &siglen, m, mlen, ctx, ctxlen, sk);
    if (r_sign != 0) {
    ...
    int r_verify = PQCLEAN_MLDSA87_CLEAN_crypto_sign_verify_ctx(sig, PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES, m, mlen, ctx, ctxlen, pk);
    if (r_verify != 0) {
...
void check_crypto_sign_signature() {
  ...
  for (int t=0; t<TESTS; t++) {
    ...
    // Sign the random message
    int r_signature = JASMIN_MLDSA87_crypto_sign_signature_ctx(sig, &siglen, m, mlen, ctx, ctxlen, sk);
    if (r_signature != 0) {
...
void check_crypto_sign_verify() {
  ...
  for (int t=0; t<TESTS; t++) {
    ...
    // Sign the random message
    int r_signature = PQCLEAN_MLDSA87_CLEAN_crypto_sign_signature_ctx(sig, &siglen, m, mlen, ctx, ctxlen, sk);
    if (r_signature != 0) {
    ...
    // Verify the maching between the signature and the message
    int r_verify = JASMIN_MLDSA87_crypto_sign_verify_ctx(sig, PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES, m, mlen, ctx, ctxlen, pk);
    if (r_verify != 0) {
