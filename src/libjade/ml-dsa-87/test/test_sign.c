...
void JASMIN_MLDSA87_crypto_sign_keypair(uint8_t *pk, uint8_t *sk);
void JASMIN_MLDSA87_crypto_sign_keypair_seed(uint8_t *pk, uint8_t *sk, uint8_t *sb);
...
void check_crypto_sign_keypair() {
  ...
  for (int t=0; t<TESTS; t++) {
    ...
    int r_sign = PQCLEAN_MLDSA87_CLEAN_crypto_sign_signature_ctx(sig, &siglen, m, mlen, ctx, ctxlen, sk_jazz);
    if (r_sign != 0) {
    ...
    int r_verify = PQCLEAN_MLDSA87_CLEAN_crypto_sign_verify_ctx(sig, PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES, m, mlen, ctx, ctxlen, pk_jazz);
    if (r_verify != 0) {
...
void check_crypto_sign_keypair_seed() {
  ...
  for (int t=0; t<TESTS; t++) {
    ...
    eqarr(PQCLEAN_MLDSA87_CLEAN_CRYPTO_SECRETKEYBYTES, PRId8, sk, sk_jazz, "crypto_sign_keypair_seed: wrong sk");
  }
...
void check_shake256_PUBLICKEYBYTES_SEEDBYTES() {
  ...
  uint8_t inp[PQCLEAN_MLDSA87_CLEAN_CRYPTO_PUBLICKEYBYTES];
  uint8_t inp_jazz[PQCLEAN_MLDSA87_CLEAN_CRYPTO_PUBLICKEYBYTES];
  for (int t=0; t<TESTS; t++) {
    fillarrnu8(inp, inp_jazz, PQCLEAN_MLDSA87_CLEAN_CRYPTO_PUBLICKEYBYTES);
    shake256(outp, SEEDBYTES, inp, PQCLEAN_MLDSA87_CLEAN_CRYPTO_PUBLICKEYBYTES);
...
void check_crypto_sign_signature_ctx_seed() {
  ...
  uint8_t sig_jazz[PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES];
  uint8_t m[mlen];
  uint8_t m_jazz[mlen];
  uint8_t ctx[ctxlen];
...
void check_crypto_sign_verify_ctx() {
  size_t siglen = PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES;
  size_t mlen = 453;
  size_t ctxlen = 239;
  uint8_t m[mlen];
  uint8_t ctx[ctxlen];
  ...
  for(int t=0; t<TESTS; t++) {
    ...
    fillarr(uint8_t, ctxlen, ctx);
    // Sign the random message
    status = PQCLEAN_MLDSA87_CLEAN_crypto_sign_signature_ctx(sig, &siglen, m, mlen, ctx, ctxlen, sk);
    if (status != 0) {
      printf("FAIL: crypto_sign_verify. Could not generate the signature.\n");
      exit(1);
    }
    // Verify the maching between the signature and the message
    int r = PQCLEAN_MLDSA87_CLEAN_crypto_sign_verify_ctx(sig, PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES, m, mlen, ctx, ctxlen, pk);
    if (r != 0) {
      printf("Could not verify signature with C code.");
      exit(1);
    }
    int r_jazz = JASMIN_MLDSA87_crypto_sign_verify_ctx(sig, PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES, m, mlen, ctx, ctxlen, pk);
    if (r != r_jazz) {
